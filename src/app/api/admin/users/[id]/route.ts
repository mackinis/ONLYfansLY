...
export async function GET(
  ...
  try {
    ...
    console.error(`Error fetching user profile ${userId} for admin:`, error);
    return NextResponse.json({ message: 'Failed to fetch user profile', error: error instanceof Error ? error.message : String(error) }, { status: 500 });
  }
}
