...
export default function LiveStreamAdminPage() {
  ...
  const [currentGeneralStreamSubtitle, setCurrentGeneralStreamSubtitle] = useState(''); // NUEVO
  ...
  const [privateCallRemoteStream, setPrivateCallRemoteStream] = useState<MediaStream | null>(null); // Stream del usuario
  const localPipVideoRef = useRef<HTMLVideoElement>(null); // PiP para el admin
  ...
  useEffect(() => {
    if (siteSettings) {
      const defaultTitleFromSettings = siteSettings.liveStreamDefaultTitle || t('adminLivestream.defaultStreamTitle');
      if (!currentGeneralStreamTitle) {
        setCurrentGeneralStreamTitle(defaultTitleFromSettings);
      }
      setLocalDefaultStreamTitle(defaultTitleFromSettings);
  ...
  }, [siteSettings, t, fetchAuthorizedUserData, authorizedUserForStream?.id, currentGeneralStreamTitle]);
  ...
  const handleEndPrivateCall = useCallback((emitToServer = true, reason = "Admin ended call") => {
    ...
    setPrivateCallRemoteStream(null); // Limpiar el stream remoto del usuario
    ...
    setPrivateCallStatus(t('adminLivestream.privateCall.callEndedTitle'));
    setHasCameraPermission(null);
    setIsMicrophoneMuted(false);
  }, [socket, authorizedUserSocketIdForCall, siteSettings?.liveStreamAuthorizedUserId, t, setIsPrivateCallActive, setPrivateCallStatus, setHasCameraPermission, setIsMicrophoneMuted]);
  ...
  useEffect(() => {
    ...
    const newSocket = io({
      path: '/api/socket_io',
      query: { appUserId: adminAppUserId },
    });
    setSocket(newSocket);
    return () => {
  ...
  useEffect(() => {
    ...
    const onConnectError = (error: Error) => {
      console.error('AdminLiveStream: Socket connection error:', error.message, error.cause, error.stack);
    ...
    const onAuthorizedUserStatus = ({ userId, isConnected, userSocketId }: { userId: string, isConnected: boolean, userSocketId: string | null }) => {
      console.log(`AdminLiveStream: Event "authorized-user-status" for UserID: ${userId}, Connected: ${isConnected}, SocketID: ${userSocketId}`);
    ...
    const onPrivateCallUserReadyForOffer = ({ userSocketId, userAppUserId }: { userSocketId: string, userAppUserId: string }) => {
      if (userAppUserId === siteSettings?.liveStreamAuthorizedUserId && isPrivateCallActive && localStreamForCallRef.current && socket && socket.connected) {
  ...
  }, [socket, siteSettings, isGeneralStreamActive, isPrivateCallActive, authorizedUserForStream, authorizedUserSocketIdForCall, t, handleEndPrivateCall]);
  ...
  const getCameraPermission = async (forCall = false): Promise<MediaStream | null> => {
    ...
    try {
      ...
      const videoEl = forCall ? localPipVideoRef.current : localVideoRef.current;
      if (videoEl) {
          if(videoEl.srcObject !== stream) videoEl.srcObject = stream;
          videoEl.muted = forCall ? true : isLocalPreviewAudioMuted; // Admin PiP is always muted for admin, general stream preview controlled by state
          await videoEl.play().catch(e => { if (e.name !== 'AbortError') console.error(`AdminLiveStream: Error playing ${forCall ? 'PiP' : 'general local'} video preview:`, e); });
      }
      return stream;
    } catch (error: any) {
      setHasCameraPermission(false);
  ...
  const handleToggleGeneralStreaming = async () => {
    ...
    if (isGeneralStreamActive) {
      ...
      if (stream) {
        ...
        if (socket.connected) {
            socket.emit('register-general-broadcaster', { streamTitle: currentGeneralStreamTitle, streamSubtitle: currentGeneralStreamSubtitle }); // Enviar subtítulo
        } else {
  ...
  const initiateWebRTCPrivateCallOffer = async (targetUserSocketId: string, currentSocket: Socket) => {
    if (!currentSocket || !currentSocket.connected || !localStreamForCallRef.current) { setPrivateCallStatus(t('adminLivestream.privateCall.statusFailed') + " (Internal Error)"); return; }
    ...
    localStreamForCallRef.current.getTracks().forEach(track => { try { peerConnectionForCallRef.current!.addTrack(track, localStreamForCallRef.current!); } catch (e:any) { console.error("Error adding track to private PC:", track.kind, e.message); }});
  ...
  const handleStartPrivateCall = async () => {
    ...
    if (stream) {
      ...
      if (socket.connected) socket.emit('admin-initiate-private-call-request', { targetUserAppId: siteSettings.liveStreamAuthorizedUserId });
        ...
        setPrivateCallStatus(t('adminLivestream.privateCall.statusFailed') + " (Socket Error)"); setIsPrivateCallActive(false); stream.getTracks().forEach(track => track.stop()); localStreamForCallRef.current = null;
        if (localPipVideoRef.current) localPipVideoRef.current.srcObject = null; setHasCameraPermission(null);
      }
  ...
  const toggleMicrophone = () => { const streamToToggle = isPrivateCallActive ? localStreamForCallRef.current : localStreamRef.current; if (streamToToggle) { const newMutedState = !isMicrophoneMuted; streamToToggle.getAudioTracks().forEach(track => track.enabled = !newMutedState); setIsMicrophoneMuted(newMutedState); toast({ title: t('adminLivestream.toast.microphoneStatusTitle'), description: newMutedState ? t('adminLivestream.toast.microphoneMuted') : t('adminLivestream.toast.microphoneUnmuted') }); }};
  const toggleLocalPreviewAudio = () => { const videoEl = isGeneralStreamActive ? localVideoRef.current : (isPrivateCallActive ? localPipVideoRef.current : null) ; if (videoEl && (isGeneralStreamActive || isPrivateCallActive)) { const newMutedState = !videoEl.muted; videoEl.muted = newMutedState; if (isGeneralStreamActive && !isPrivateCallActive) setIsLocalPreviewAudioMuted(newMutedState); toast({ title: t('adminLivestream.toast.localAudioStatusTitle'), description: newMutedState ? t('adminLivestream.toast.localAudioMuted') : t('adminLivestream.toast.localAudioUnmuted')}); }};


  useEffect(() => { // General stream local preview
    ...
    if (videoEl) {
      ...
      } else if (videoEl.srcObject && !isGeneralStreamActive) {
  ...
  useEffect(() => { // Admin's PiP during private call
    ...
    if (videoEl) {
        if (isPrivateCallActive && localStreamForCallRef.current) {
            if (videoEl.srcObject !== localStreamForCallRef.current) {
                videoEl.srcObject = localStreamForCallRef.current;
                videoEl.muted = true; // Admin's own PiP should always be muted for the admin
                videoEl.play().catch(e => { if (e.name !== 'AbortError') console.error("Error playing admin PiP video:", e); });
            }
        } else if (videoEl.srcObject) {
            videoEl.srcObject = null;
        }
    }
  }, [isPrivateCallActive, localStreamForCallRef]);
    ...
    if (videoEl) {
      if (isPrivateCallActive && privateCallRemoteStream) {
        if (videoEl.srcObject !== privateCallRemoteStream) {
          ...
          videoEl.muted = false; // Remote stream should not be muted
  ...
  console.log("AdminLiveStream: Checking canStartPrivateCall conditions:", {
    liveStreamAuthorizedUserId: siteSettings?.liveStreamAuthorizedUserId,
    isAuthorizedUserConnected,
    authorizedUserSocketIdForCall,
    isPrivateCallActive,
    isGeneralStreamActive,
    calculatedCanStart: !!(siteSettings?.liveStreamAuthorizedUserId && isAuthorizedUserConnected && authorizedUserSocketIdForCall && !isPrivateCallActive && !isGeneralStreamActive)
  });
  ...
  let videoAreaSubtitle = currentGeneralStreamSubtitle; // Subtítulo para stream general

  if (isPrivateCallActive) {
    mainVideoPlayerSrcObject = privateCallRemoteStream;
    mainVideoPlayerIsMuted = false; // El stream remoto del usuario no se silencia para el admin
    videoAreaTitle = t('adminLivestream.remoteUserVideoLabel');
    videoAreaSubtitle = ''; // No hay subtítulo para la vista de llamada privada
    ...
    // videoAreaTitle y videoAreaSubtitle ya están seteados para stream general
  }

  let isLoadingStateForVideoArea = false;
  if (isGeneralStreamActive && !localStreamRef.current && hasCameraPermission === null) {
    isLoadingStateForVideoArea = true;
  } else if (isPrivateCallActive && !privateCallRemoteStream && localStreamForCallRef.current && (privateCallStatus === t('adminLivestream.privateCall.statusConnecting', { userName: authorizedUserForStream?.name || 'User' }) || privateCallStatus === t('adminLivestream.privateCall.statusIncoming'))) {
    isLoadingStateForVideoArea = true;
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Card para Configuración de Stream General */}
      ...
      {/* Card para Videollamada Privada */}
      <Card className="shadow-xl border-primary/20">
        <CardHeader>
      ...
      {/* Área de Visualización de Video Principal */}
      <Card className="shadow-xl border-primary/20">
        <CardHeader>
        ...
        <CardContent>
          <div className="relative bg-background/50 p-2 sm:p-4 rounded-lg border border-border shadow-inner">
            <video
              ref={isPrivateCallActive ? remoteVideoRef : localVideoRef}
            ...
            {isPrivateCallActive && localStreamForCallRef.current && (
              <div className="absolute bottom-4 right-4 w-1/3 sm:w-1/4 aspect-video border-2 border-primary rounded-md overflow-hidden shadow-lg z-10">
            ...
            {isLoadingStateForVideoArea && (
                 <div className="absolute inset-0 flex flex-col items-center justify-center bg-muted/80 rounded-md pointer-events-none">
                    ...
                    <p className="text-md text-muted-foreground">{privateCallStatus || t('adminLivestream.videoArea.startingCamera')}</p>
                 </div>
            )}
             {!isAnyStreamAttemptingOrActive && !isLoadingStateForVideoArea &&(
                 <div className="absolute inset-0 flex flex-col items-center justify-center bg-muted/80 rounded-md pointer-events-none">
           ...
           {(isGeneralStreamActive || isPrivateCallActive) && (localStreamRef.current || localStreamForCallRef.current) && (
              <div className="mt-3 flex items-center justify-center space-x-2 sm:space-x-3">
                ...
                {isGeneralStreamActive && !isPrivateCallActive && ( // Solo mostrar para stream general
      ...
      {/* Card para Configuraciones Persistentes */}
      <Card className="shadow-xl border-primary/20">
        <CardHeader>
